...
from .models import Panel, PanelComment, Panelist, PanelRequest, PanelSlot, Room, Track
...
def ControlsV1GetPanels(request, event_id):
    ...
    for x in panels:
        ...
        output[str(x.pk)] = {
            ...
            "title": str(x.label),
            "time_start": str(
                timezone.localtime(x.time_start).strftime("%a %I:%M%p %Z")
            ),
            ...
            "setup": str(
                (timezone.timedelta(minutes=x.setup_time).days * 24)
                + timezone.timedelta(minutes=x.setup_time).seconds // 3600
            )
            + " Hours, "
            + str((timezone.timedelta(minutes=x.setup_time).seconds // 60) % 60)
            + " Minutes",
            "setup_blocks": x.setup_time / 15,
            "duration": str(
                (timezone.timedelta(minutes=x.duration).days * 24)
                + timezone.timedelta(minutes=x.duration).seconds // 3600
            )
            + " Hours, "
            + str((timezone.timedelta(minutes=x.duration).seconds // 60) % 60)
            + " Minutes",
...
def ControlsV1GetEventDetail(request, event_id):
    ...
    while i <= days:
        names.append(
            (
                (event.eventStart + timezone.timedelta(days=i)).strftime("%A"),
                (event.eventStart + timezone.timedelta(days=i)).strftime("%m%d"),
            )
        )
        names_full.append(
            (
                (event.eventStart + timezone.timedelta(days=i)).strftime("%A"),
                (event.eventStart + timezone.timedelta(days=i)).strftime("%m%d%Y"),
            )
        )
        day_blocks[
            (event.eventStart + timezone.timedelta(days=i - 1)).strftime("%m%d")
        ] = i
        reverse_day_blocks[i] = (
            event.eventStart + timezone.timedelta(days=i - 1)
        ).strftime("%m%d")
        i = i + 1
    ...
    reverse_day_blocks[i + 1] = (
        event.eventStart + timezone.timedelta(days=i)
    ).strftime("%m%d")
...
def testform(request, event_id):
    ...
    return render(
        request, "testform.html", {"events": events, "event": event, "form": form}
    )
...
def manager(request, event_id, track_id=None):
    ...
    if track_id != None:
        ...
        filter = ": " + track.title
    context = {"event": event, "events": events, "list": panels, "filter": filter}
...
def checkIn(request, event_id):
    ...
    for x in panelists:
        pnx[str(x.pk)] = {
            "panelist": x,
            "panels": PanelSlot.objects.filter(
                event=event, panel__panelist=x, panel__type=0
            ).order_by("time_start"),
        }
    context = {
        "event": event,
...
def ControlsV1PullPanelVerification(request, event_id, panel_id):
    ...
    m_panel_time_end = timezone.localtime(
        panel.time_start + timezone.timedelta(minutes=panel.duration)
    )
    m_panel_start_setup = timezone.localtime(
        panel.time_start - timezone.timedelta(minutes=panel.setup_time)
    )
    output = {
        "panel": {},
        "panel_before": {},
    ...
    output["panel"] = {
        ...
        "room": str(panel.room),
        "time_start": str(
            timezone.localtime(panel.time_start).strftime("%a %I:%M%p %Z")
        ),
        "time_end": str(
            timezone.localtime(
                panel.time_start + timezone.timedelta(minutes=panel.duration)
        ...
        for x in PanelSlot.objects.filter(event=event, room=y).order_by("time_start"):
            ...
            if x == panel:
                ...
                time_end = timezone.localtime(
                    panel_before.time_start
                    + timezone.timedelta(minutes=panel_before.duration)
                )
                time_start_setup = timezone.localtime(
                    panel_before.time_start
                    - timezone.timedelta(minutes=panel_before.setup_time)
                )
                try:
                    if panel_before == panel:
                        ...
                        output["panel_before"] = {
                            ...
                            "time_start": str(
                                timezone.localtime(panel_before.time_start).strftime(
                                    "%a %I:%M%p %Z"
                                )
                            ),
                            "time_end": str(
                ...
                if (
                    panel.time_start > time_start_setup
                    and panel.time_start < time_end
                    and panel != x
                ):
                    output["conflicts"].append(
                        "Panel starts during Previous Panel ("
                        + panel_before.panel.title
                        + ")"
                    )

                # 2. check if setup start is during before
                if (
                    m_panel_start_setup > time_start_setup
                    and m_panel_start_setup < time_end
                    and panel != x
                ):
                    output["conflicts"].append(
                        "Panel Setup time starts during Previous Panel ("
                        + panel_before.panel.title
                        + ")"
                    )
            ...
            if beforei == 0:
                panel_after = x
                time_end = timezone.localtime(
                    panel_after.time_start
                    + timezone.timedelta(minutes=panel_after.duration)
                )
                time_start_setup = timezone.localtime(
                    panel_after.time_start
                    - timezone.timedelta(minutes=panel_after.setup_time)
                )
                try:
                    output["panel_after"] = {
                        ...
                        "time_start": str(
                            timezone.localtime(panel_after.time_start).strftime(
                                "%a %I:%M%p %Z"
                            )
                        ),
                        "time_end": str(
                ...
                if panel.time_start > time_start_setup and panel.time_start < time_end:
                    output["conflicts"].append(
                        "Panel starts during Following Panel ("
                        + panel_after.title
                        + ")"
                    )
                ...
                if (
                    ...
                    output["conflicts"].append(
                        "Panel Setup time starts during Following Panel ("
                        + panel_after.title
                        + ")"
                    )
                ...
                if m_panel_time_end > time_start_setup:
                    output["conflicts"].append(
                        "Panel Ends During or After Following Panel ("
                        + panel_after.title
                        + ")"
                    )

            time_start_setup = timezone.localtime(
                x.time_start - timezone.timedelta(minutes=x.setup_time)
            )
            time_end = timezone.localtime(
                x.time_start + timezone.timedelta(minutes=x.duration)
            )
                    ...
                    output["adjs"].append(
                        {
                            ...
                            "time_start": str(
                                timezone.localtime(x.time_start).strftime(
                                    "%a %I:%M%p %Z"
                                )
                            ),
                            "time_end": str(
                                timezone.localtime(
                                    x.time_start
                                    + timezone.timedelta(minutes=x.duration)
                                ).strftime("%a %I:%M%p %Z")
                            ),
            ...
            if (
                m_panel_time_end >= time_start_setup
                and m_panel_time_end <= time_end
                and x.panel.type != 3
                and checked == 0
            ):
                checked = 1
    ...
    for y in Room.objects.filter(event=event):
        ...
        genui.append(
            (
                datecode,
                True,
                str(timezone.localtime(now).strftime("%a %I:%M%p %Z")),
                y.pk,
            )
        )
        for x in PanelSlot.objects.filter(event=event, room=y):
            ...
            if now >= timezone.localtime(
                x.time_start - timezone.timedelta(minutes=x.setup_time)
            ) and now <= timezone.localtime(x.time_start):
                in_setup = True
                happens = True

            if now >= timezone.localtime(x.time_start) and now <= timezone.localtime(
                x.time_start + timezone.timedelta(minutes=x.duration)
            ):
                happens = True
            ...
            outp = {
                ...
                "time_slot": timezone.localtime(x.time_start).strftime("%m%d%H%M"),
                "setup": str(timezone.timedelta(minutes=x.setup_time).seconds // 3600)
                + " Hours, "
                + str((timezone.timedelta(minutes=x.setup_time).seconds // 60) % 60)
                + " Minutes",
                "duration": str(timezone.timedelta(minutes=x.duration).seconds // 3600)
                + " Hours, "
                + str((timezone.timedelta(minutes=x.duration).seconds // 60) % 60)
                + " Minutes",
                "r18": x.panel.r18,
            ...
            genui.append(
                (
                    timezone.localtime(x.time_start).strftime("%m%d%H%M"),
                    False,
                    outp,
                    y.pk,
                )
...
def ControlsV1PullEventsTimecode(request, event, timecode):
    ...
    now = timezone.localtime(
        timezone.make_aware(datetime.datetime.strptime(timecode, "%m%d%Y%H%M"))
    )
    # now = timezone.localtime(timezone.now())
    output = {}
    # Am I in the middle of an event?
