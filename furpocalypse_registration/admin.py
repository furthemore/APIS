...
from furpocalypse_registration import mqtt, payments
from furpocalypse_registration.forms import FirebaseForm
...
def send_staff_token_email(modeladmin, request, queryset):
    for token in queryset:
        furpocalypse_registration.emails.send_new_staff_email(token)
        token.sent = True
        token.save()
    if queryset.count() > 1:
        messages.success(
...
def send_approval_email(modeladmin, request, queryset):
    furpocalypse_registration.emails.send_dealer_approval_email(queryset)
...
def send_payment_email(modeladmin, request, queryset):
    for dealer in queryset:
        ...
        if oi and oi.order:
            furpocalypse_registration.emails.send_dealer_payment_email(dealer, oi.order)
    if queryset.count() > 1:
        messages.success(request, "Successfully emailed %d dealers" % queryset.count())
    else:
        messages.success(request, "Successfully emailed %s" % queryset[0].attendee)
...
def send_assistant_form_email(modeladmin, request, queryset):
    for dealer in queryset:
        furpocalypse_registration.emails.send_dealer_assistant_form_email(dealer)
    if queryset.count() > 1:
        messages.success(request, "Successfully emailed %d dealers" % queryset.count())
    else:
        messages.success(request, "Successfully emailed %s" % queryset[0].attendee)
...
def send_assistant_registration_email(modeladmin, request, queryset):
    for assistant in queryset:
        furpocalypse_registration.emails.send_dealer_assistant_registration_invite(
            assistant
        )
    if queryset.count() > 1:
        messages.success(
...
def send_staff_registration_email(modeladmin, request, queryset):
    for staff in queryset:
        furpocalypse_registration.emails.send_staff_promotion_email(staff)
...
def send_upgrade_form_email(modeladmin, request, queryset):
    for badge in queryset:
        furpocalypse_registration.emails.send_upgrade_instructions(badge)
    if queryset.count() > 1:
        messages.success(
...
def resend_confirmation_email(modeladmin, request, queryset):
    for badge in queryset:
        ...
        furpocalypse_registration.emails.send_registration_email(
