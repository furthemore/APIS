...
def send_registration_email(order, email, send_vip=True):
    ...
    for oi in order_items:
        registration_email = furpocalypse_registration.views.common.get_registration_email(
            oi.badge.event
        )
...
def send_staff_registration_email(orderId):
    ...
    event = Event.objects.get(default=True)
    staff_email = furpocalypse_registration.views.staff.get_staff_email(event)
    send_email(
        staff_email,
        [email],
...
def send_staff_promotion_email(staff):
    ...
    staff_email = furpocalypse_registration.views.staff.get_staff_email(staff.event)
    send_email(
        staff_email,
...
def send_dealer_application_email(dealerId):
    ...
    dealer_email = furpocalypse_registration.views.dealers.get_dealer_email(dealer.event)
    send_email(
        dealer_email,
...
def send_dealer_assistant_registration_invite(assistant):
    ...
    dealer_email = furpocalypse_registration.views.dealers.get_dealer_email(
        assistant.event
    )
    send_email(
        dealer_email,
