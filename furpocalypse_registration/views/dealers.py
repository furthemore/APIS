...
def add_assistants_checkout(request):
    ...
    if status:
        ...
        clear_session(request)
        try:
            furpocalypse_registration.emails.send_dealer_assistant_email(dealer.id)
            ...
            for assistant in dealer.dealerasst_set.all().filter(attendee__isnull=True):
                furpocalypse_registration.emails.send_dealer_assistant_registration_invite(assistant)
...
def checkout_dealer(request):
    ...
    if subtotal == 0:
        ...
        try:
            furpocalypse_registration.emails.send_dealer_payment_email(dealer, order)
        except Exception as e:
