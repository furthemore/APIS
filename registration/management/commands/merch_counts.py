...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        selection = self.prompt_int("Enter index of event to report on [{0}] > ".format(default_event.name), True)
        ...
        if selection is None:
            return
        selected_options = AttendeeOptions.objects.filter(option=options[selection], orderItem__badge__event=event) 
        bins = {}
        staff = 0
        levels = {}
        print("{0} orders with {1} option selected for {2}".format(selected_options.count(), options[selection], event))
        for attendee_option in selected_options:
            ...
            if attendee_option.option.optionExtraType == "ShirtSizes":
                assert str(attendee_option.optionValue) in SHIRT_SIZES.keys(), "Invalid response in AttendeeOption(id={0})".format(attendee_option.id)
                try:
