...
from staff.models import Department, StaffApplicant, StaffPosting, Staff, StaffInvite, StaffEventRegistration
        ...
        for dept in led_departments:
            ...
            for staff_member in Staff.objects.filter(department=dept).select_related('user'):
                ...
                staff_list.append({
                    ...
                    'checked_in': event_registration.checked_in if event_registration else False,
                })
...
def register(request, token):
    ...
    try:
        ...
        return render(request, 'staff/register_error.html', {
    ...
    if request.method == 'POST':
        ...
        last_reg = StaffEventRegistration.objects.filter(
            staff=staff_profile
        ).exclude(event=event).order_by('-event__eventStart').first()
