...
class Volunteer(models.Model):
	...
	def onJob(self):
		if Job.objects.filter(volunteer=self,timeOut=None).order_by("-timeIn").first() == None:
			return False
		return True
	def lastJob(self):
		...
		if j.detail == "":
			j.detail = "No Details"
		return str(timezone.localtime(j.timeIn).strftime("%m/%d/%Y %H:%M"))+" - "+str(j.detail)
...
class Job(models.Model):
	...
	staffIn = models.ForeignKey('registration.Staff', blank=True, null=True, related_name='staffIn')
	staffOut = models.ForeignKey('registration.Staff', blank=True, null=True, related_name='staffOut')
	...
	def duration(self):
		try:
			...
			return (timezone.localtime(timezone.now()) - timezone.localtime(self.timeIn)) * self.multiplier
	def __str__(self):
