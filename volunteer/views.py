...
def ControlsV1EventVolunteerCheckIn(request, event_id, volunteer_id):
	event = get_object_or_404(Event, pk=event_id)
	x = get_object_or_404(Volunteer, pk=str(volunteer_id).replace("BRCD_"+event_id,""))
...
def ControlsV1EventVolunteerTimesheet(request, event_id, volunteer_id):
	...
	for y in Job.objects.filter(volunteer=x).order_by('timeIn'):
		output[str(x.pk)].append( (
					...
					str((y.duration().days * 24)+y.duration().seconds//3600)+" Hours, "+str((y.duration().seconds//60)%60)+" Minutes", 
					y.detail,
					y.multiplier,
					str(y.staffIn),
					str(y.staffOut),
	...
	output['total'] = str((totals.days * 24)+totals.seconds//3600)+" Hours, "+str((totals.seconds//60)%60)+" Minutes"
	return JsonResponse(output)
